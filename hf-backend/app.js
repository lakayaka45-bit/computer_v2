...
const { insertDemoData } = require('./utills/insertDemoData');
...
const ensureDatabaseExists = async () => {
  const databaseUrl = process.env.DATABASE_URL || 'mysql://root:@127.0.0.1:3306/computer_shop?connection_limit=5';
  ...
  try {
    ...
    connection.end();
    console.log(`✅ Ensured MySQL database exists: ${databaseName}`);
  } catch (error) {
    console.warn('⚠️ Could not ensure MySQL database exists:', error.message);
  }
};

const runPrismaSetup = () => {
  try {
    const result = spawnSync('npx', ['prisma', 'db', 'push', '--accept-data-loss', '--schema=./prisma/schema.prisma', '--skip-generate'], {
      cwd: __dirname,
      stdio: 'inherit',
      env: process.env,
    });
    ...
    if (result.status !== 0) {
      console.warn('⚠️ Prisma db push completed with warnings or errors. Continuing startup.');
    } else {
      console.log('✅ Prisma database schema is up to date');
    }
  } catch (setupError) {
    console.warn('⚠️ Prisma setup step failed:', setupError.message);
  }
};

const seedDemoDataWithRetry = async (retries = 3) => {
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      await insertDemoData();
      console.log('✅ Demo data seed check completed successfully');
      return;
    } catch (seedError) {
      console.warn(`⚠️ Demo data seed attempt ${attempt}/${retries} failed:`, seedError.message);
      if (attempt === retries) {
        throw seedError;
      }
...
const startServer = async () => {
  try {
    ...
    runPrismaSetup();
    await seedDemoDataWithRetry();
  } catch (seedError) {
    console.error('⚠️ Demo data seed failed:', seedError);
  }
